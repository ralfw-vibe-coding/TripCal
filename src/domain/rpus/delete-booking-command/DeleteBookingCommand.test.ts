...
import { DeleteBookingCommand } from "./DeleteBookingCommand";
...
describe("DeleteBookingCommand", () => {
  it("records a deletion event and hides the booking from the calendar", async () => {
    const eventStore = new MemoryEventStore();
    const ids = new FixedIds(["text-1", "booking-1", "delete-1"]);
    ...
    const deleted = await new DeleteBookingCommand(eventStore, ids, new FixedClock()).process({
      bookingExtractedId: "booking-1",
    });

    expect(deleted).toEqual({ status: "succeeded", bookingDeletedId: "delete-1" });
